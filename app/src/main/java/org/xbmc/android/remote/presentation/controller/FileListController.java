...
public class FileListController extends ListController implements IController {
	private static final String TAG = "FileListController";
	...
	public void onCreate(Activity activity, Handler handler, AbsListView list) {
		LOG.I(TAG, "FileListController.onCreate(): mediaType = " + mMediaType);
		...
		if (!isCreated()) {
			...
			mList.setOnItemClickListener(new OnItemClickListener() {
				public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                    LOG.I(TAG, "FileListController.mList.setOnItemClickListener(): position = " + position);
					if (isLoading())
                        return;
					if (mFileItems == null)
						return;
					...
					if (item.isDirectory) {
                        LOG.I(TAG, "Item is Directory: starting new ListActivity. Path = " + item.path);
...
//								break;
	...
	private void fillUp(final String url, final String displayPath) {
		LOG.D(TAG, "fillUp: url = " + url + ", displayPath = " + displayPath);
		if (mGettingUrl != null)
			return;
	...
	public void onContextItemSelected(MenuItem item) {
		...
		switch(item.getItemId()) {
		    case ITEM_CONTEXT_QUEUE:
			    LOG.D(TAG, "Item Queue: file location = " + loc.path + ", playlistid = " + playlistid);
		    ...
		    case ITEM_CONTEXT_PLAY:
			    LOG.D(TAG, "Item Play: file location = " + loc.path + ", playlistid = " + playlistid);
			    mControlManager.playFile(new DataResponse<Boolean>() {
				    public void run() {
				        if (value) {
					        mActivity.startActivity(new Intent(mActivity, NowPlayingActivity.class));
			    ...
			    }, loc.path, playlistid, mActivity.getApplicationContext());
                break;
		}
	}
